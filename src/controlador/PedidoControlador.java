...
public class PedidoControlador {
    ...
    public PedidoControlador(BienvenidaVista bv, TipoConsumoPagoVista cv, InicioVista iv, ProcesoPagoVista pv, CocinaVista cocv, PizzaFactory factory, PizzaBuilder builder, PedidoFacade facade) {
...
public double calcularPrecioFinalActual() {
    ...
    public double calcularTotal(String tipoPizza, String tamanio, List<Ingrediente> seleccionados, int cantidad) {
        ...
        for (Ingrediente ing : seleccionados) {
            boolean yaIncluido = pizza.getIngredientesBase().stream().anyMatch(base -> base.getNombre().equalsIgnoreCase(ing.getNombre()));
...
private void guardarComprobanteTxt(Pedido pedido) {
    try (java.io.PrintWriter out = new java.io.PrintWriter("comprobante_" + pedido.getId() + ".txt")) {
        ...
        out.println("Masa: " + pedido.getMasa()); // ✅ ahora muestra masa
        ...
        out.println("Total: S/ " + String.format("%.2f", pedido.calcularTotal())); // ✅ total corregido
        out.println("--------------------------------------");
    } catch (Exception e) {
